...
/**
 * Heading-derived table of contents with IntersectionObserver scroll-spy — the
 * document analog of open-slide's thumbnail rail / overview grid. Scans the
 * rendered reading column for headings with ids (set by the Heading primitive).
 */
type Entry = { id: string; text: string; level: number };

export function Toc({ docKey }: { docKey: string }) {
  ...
  useEffect(() => {
    ...
    // Re-scan when the document changes (id) or its content hot-reloads.
  }, [docKey]);

  if (!entries.length) return <nav className="odc-toc" aria-label="Table of contents" />;
