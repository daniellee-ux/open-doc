 ...
 * `SlideModule`. A document is an ordered array (array order == reading order),
 * rendered CONCATENATED into one continuous flow rather than one-at-a-time on a
 * scaled canvas. Section metadata lives as static properties on the component
 * (like open-slide's `Page.transition`); `DocBody` reads them at render time.
 */
export type Section = FC & {
  /** Stable anchor id for TOC links + scroll-spy. Defaults to `section-{n}`. */
  id?: string;
  /** Force a print page break before this section (`break-before: page`). */
  breakBefore?: boolean;
